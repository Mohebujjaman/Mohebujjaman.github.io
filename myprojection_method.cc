 ...
 *-------------------------------------------------------------------*/
    ...
    struct RightSideScratchData
    {
      ...
      RightSideScratchData (const RightSideScratchData &data)
        :
        nqp (data.nqp),
        fe_val_vel (data.fe_val_vel.get_fe(), data.fe_val_vel.get_quadrature(), data.fe_val_vel.get_update_flags())
...
/* -------------------------------------------------------------------
 * Here we want to compute pressure error squre. To do so, we compute 
...
/* -------------------------------------------------------------------
 * As we want to compute |p-p_{mean}-(p_h-p_{hmean}| which is equal to
 * |p-(p_h-p_{hmean}+p_{mean}|
 * Therefore, we are subtracting p_{hmean} and adding p_{mean} from the
 * finite dimensional pressure.
 * Note that: p_{hmean} is for mean of finite dimensional pressure and
 * p_{mean} is for mean of interpolating pressure.
 *
 *-------------------------------------------------------------------*/
...
//    The following code segments are exactly similar things for H1
  ...
  { 
    ...
    InitGradScratchData scratch_data (fe_velocity,
                                      fe_pressure,
                                      quadrature_velocity,
                                      update_gradients | update_JxW_values | update_values|update_quadrature_points ,
                                      update_values);
...
// advection term assembling and gradient vector assembling
  ...
  { 
    ...
    //settting time in exact velocity function
